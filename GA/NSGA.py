...
class NSGAOptimizationTracker:
    ...
    def plot_2d_curves(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_2d_curves.png', dpi=300, bbox_inches='tight')
...
def multi_objective_optimization(irrigation_system, lgz1, lgz2, swarm_size, max_iterations, show_plots, auto_save):
    ...
    def evaluate(individual):
        ...
        try:
            ...
            # 关键修复：将最终的正确基因编码更新到原individual中
    ...
    try:
        ...
        save_pareto_front(pareto_front_values, "NSGA_DAN_Verified")
        save_pareto_solutions(verified_pareto_front, "NSGA_DAN_Verified")
        print("验证通过的帕累托解集已成功保存")

        # 如果有失败的个体，也保存一份用于分析
        if failed_individuals:
            failed_values = np.array([ind.fitness.values for ind in failed_individuals])
            save_pareto_front(failed_values, "NSGA_DAN_Failed")
            save_pareto_solutions(failed_individuals, "NSGA_DAN_Failed")
...
def verify_individual_correctness(individual, irrigation_system, group_count):
    ...
    try:
        ...
        # 检查计算结果与记录的适应度值是否一致
        cost_diff = abs(avg_cost - individual.fitness.values[0])
        var_diff = abs(avg_variance - individual.fitness.values[1])
...
def print_detailed_results(irrigation_system, best_individual, output_file="optimization_results_NSGAⅡ_DAN.txt"):
    """优化后的结果输出函数，包含压力分析（使用节点基准水头）"""

    # 【关键修改】：使用优化后的基因编码（如果存在）
    print("正在解码最优解的管径配置...")
    if hasattr(best_individual, 'optimized_genes') and best_individual.optimized_genes is not None:
        print("使用优化后的基因编码进行解码...")
        decode_solution_to_pipe_diameters(irrigation_system, best_individual.optimized_genes)
    else:
        print("使用原始基因编码进行解码...")
