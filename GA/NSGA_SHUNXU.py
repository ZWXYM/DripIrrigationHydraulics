...
class NSGAOptimizationTracker:
    ...
    def finalize_plots(self):
        ...
        try:
            ...
            # 如果需要，保存图表
            if self.auto_save:
                self.fig_2d.savefig('NSGA_DAN_SHUNXU_2d_curves.png', dpi=300, bbox_inches='tight')
                self.fig_3d.savefig('NSGA_DAN_SHUNXU_3d_progress.png', dpi=300, bbox_inches='tight')
    ...
    def plot_2d_curves(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_SHUNXU_2d_curves.png', dpi=300, bbox_inches='tight')
    ...
    def plot_3d_progress(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_SHUNXU_3d_progress.png', dpi=300, bbox_inches='tight')
...
def multi_objective_optimization(irrigation_system, lgz1, lgz2, swarm_size, max_iterations, show_plots, auto_save):
    ...
    def adjust_pipe_diameters_holistic(individual, irrigation_system, all_groups):
        """
        新增函数 - 适用于【方案二：顺序分组】的鲁棒性调整
        功能：
        对一个给定的管径设计（individual），全面检查其是否满足所有轮灌组（all_groups）的压力要求。
        如果不满足，则迭代式地、整体性地调大不满足要求的管段的管径，直到所有组的所有节点都满足压力要求。
        这种方法避免了“满足一个组，破坏另一个组”的问题。

        返回：
        (bool, list): (是否成功找到可行解, 最终的管径基因列表)
        """
        ...
        while attempt < max_attempts:
            _update_pipe_diameters(irrigation_system, working_individual)
            ...
            for seg_index in sorted_segments:
                ...
                if larger_diameters:
                    ...
                    # 直接在 working_individual 中修改基因
                    # individual 的索引比管段索引小1
                    ...
                    if gene_index < len(irrigation_system.main_pipe) - 1:
                        ...
                        if working_individual[gene_index] < new_gene_value:
                            working_individual[gene_index] = new_gene_value
        ...
        return False, working_individual  # 超过最大尝试次数，宣告失败

    def evaluate1(individual):
        """
        评估函数 - 适用于【方案二：顺序分组】
        修改说明：
        - 【修正】修复了原代码中调用 irrigation_system.evaluate_group 时传递参数类型错误的bug。
        - 循环应使用索引 `group_idx`，而不是直接传递轮灌组列表 `group`。
        """
        try:
            # 获取所有轮灌组
            ...
            # 使用新的整体调整函数
            success, adjusted_individual = adjust_pipe_diameters_holistic(individual, irrigation_system, all_groups)

            if not success:
                return float('inf'), float('inf')

            # 关键：将最终验证通过的、正确的基因编码更新到原 individual 中
            individual[:] = adjusted_individual

            # 使用最终确定的、对所有组都有效的管径方案，来计算各项指标
            _update_pipe_diameters(irrigation_system, adjusted_individual)

            total_cost = 0
            pressure_variances = []

            # 再次遍历所有组，这次是为了计算指标，而不是为了调整
            # --- BUG 修正处 ---
            # 使用索引进行迭代，以匹配 `evaluate_group` 方法的参数要求
            ...
            avg_pressure_variance = np.mean(pressure_variances) if pressure_variances else float('inf')

            return final_cost, avg_pressure_variance
    ...
    try:
        ...
        save_pareto_front(pareto_front_values, "NSGA_DAN_SHUNXU_Verified")
        save_pareto_solutions(verified_pareto_front, "NSGA_DAN_SHUNXU_Verified")
        print("验证通过的帕累托解集已成功保存")

        # 如果有失败的个体，也保存一份用于分析
        if failed_individuals:
            failed_values = np.array([ind.fitness.values for ind in failed_individuals])
            save_pareto_front(failed_values, "NSGA_DAN_SHUNXU_Failed")
            save_pareto_solutions(failed_individuals, "NSGA_DAN_SHUNXU_Failed")
...
def save_pareto_front(pareto_front, algorithm_name, save_dir='NSGA_DAN_SHUNXU_result'):
    """
...
def save_pareto_solutions(solutions, algorithm_name, save_dir='NSGA_DAN_SHUNXU_result'):
    """
...
def print_detailed_results(irrigation_system, best_individual, output_file="optimization_results_NSGA_DAN_SHUNXU.txt"):
    """
...
def visualize_pareto_front(pareto_front):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('NSGA_DAN_SHUNXU_pareto_front.png', dpi=300, bbox_inches='tight')
    except Exception as e:
    ...
    try:
        ...
        if auto_save == 1:
            ...
            for i, fig in enumerate(figures):
                try:
                    fig.savefig(f'NSGA_DAN_SHUNXU_result_fig{i}.png', dpi=300, bbox_inches='tight')
                    logging.info(f"图表{i}已保存为 NSGA_DAN_SHUNXU_result_fig{i}.png")
                except Exception as e:
