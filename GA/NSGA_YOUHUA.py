...
class NSGAOptimizationTracker:
    ...
    def finalize_plots(self):
        ...
        try:
            ...
            # 如果需要，保存图表
            if self.auto_save:
                self.fig_2d.savefig('NSGA_DAN_YOUHUA_2d_curves.png', dpi=300, bbox_inches='tight')
                self.fig_3d.savefig('NSGA_DAN_YOUHUA_3d_progress.png', dpi=300, bbox_inches='tight')
    ...
    def plot_2d_curves(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_YOUHUA_2d_curves.png', dpi=300, bbox_inches='tight')
    ...
    def plot_3d_progress(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_YOUHUA_3d_progress.png', dpi=300, bbox_inches='tight')
...
def multi_objective_optimization(irrigation_system, lgz1, lgz2, swarm_size, max_iterations, show_plots, auto_save):
    ...
    def evaluate(individual):
        """
        评估函数 - 适用于【原始方案：头尾对开】，并采用【全局合并方差法】
        说明：
        此函数是NSGA-II算法的核心，用于计算每个个体的帕累托目标值。
        - 保留了原始方案中，逐个轮灌组迭代调整管径的核心算法。
        - 将最终的性能评价指标（压力均方差），从“各组方差的平均值”修改为更公平的“全局合并方差法”。
        """
        try:
            # 步骤一：采用原始算法，找到一个能满足所有轮灌组的管径设计
            # 这个过程与原始代码完全相同
            working_individual = list(individual)
            all_groups_success = True
            final_corrected_individual = None
            group_count = len(irrigation_system.irrigation_groups)

            for group_idx in range(group_count):
                active_nodes = irrigation_system.irrigation_groups[group_idx]
                # 调用原始的、逐组调整的管径修正函数
                success, adjusted_individual = adjust_pipe_diameters(working_individual, active_nodes)

                if not success:
                    all_groups_success = False
                    break

                working_individual = adjusted_individual
                final_corrected_individual = adjusted_individual

            if not all_groups_success:
                return float('inf'), float('inf')

            # 确保最终的基因与系统状态一致
            if final_corrected_individual is not None:
                individual[:] = final_corrected_individual
            else:
                _update_pipe_diameters(irrigation_system, individual)
                final_corrected_individual = encode_pipe_diameters_from_system(irrigation_system)
                individual[:] = final_corrected_individual

            # 步骤二：使用最终确定的管径方案，计算成本和全局压力均方差
            _update_pipe_diameters(irrigation_system, final_corrected_individual)

            # 【核心修改：采用全局合并方差法计算第二个目标值】
            ...
            # 对收集到的所有数据点进行一次性标准差计算 (第二个目标值)
            if not all_pressure_surpluses:
                global_std_dev = float('inf')
            else:
    ...
    try:
        ...
        save_pareto_front(pareto_front_values, "NSGA_DAN_YOUHUA_Verified")
        save_pareto_solutions(verified_pareto_front, "NSGA_DAN_YOUHUA_Verified")
        print("验证通过的帕累托解集已成功保存")

        # 如果有失败的个体，也保存一份用于分析
        if failed_individuals:
            failed_values = np.array([ind.fitness.values for ind in failed_individuals])
            save_pareto_front(failed_values, "NSGA_DAN_YOUHUA_Failed")
            save_pareto_solutions(failed_individuals, "NSGA_DAN_YOUHUA_Failed")
...
def save_pareto_front(pareto_front, algorithm_name, save_dir='NSGA_DAN_YOUHUA_result'):
    """
...
def save_pareto_solutions(solutions, algorithm_name, save_dir='NSGA_DAN_YOUHUA_result'):
    """
...
def verify_individual_correctness(individual, irrigation_system, group_count):
    ...
    try:
        ...
        stored_std_dev = individual.fitness.values[1]
...
def print_detailed_results(irrigation_system, best_individual,
                           output_file="optimization_results_NSGA_DAN_YOUHUA.txt"):
    """
    优化后的结果输出函数 - 【完整通用版】
    说明：
    - 本版本包含了所有输出细节，无任何省略。
    - 采用【全局合并方差法】进行最终的性能评价，可公平地用于任何多组轮灌方案（如方案二或原始方案）。
    - 建议根据您正在运行的方案，修改默认的 output_file 文件名以作区分。
    """
    # 解码最优个体，将管径配置应用到系统中
    print("正在解码最优解的管径配置...")
    ...
    # 获取每个节点的基准水头需求，用于后续计算
    baseline_heads_data = irrigation_system.calculate_node_baseline_heads()
    baseline_heads = {data['node']: data['baseline_head'] for data in baseline_heads_data}

    # 打开文件，准备写入报告
    with open(output_file, 'w', encoding='utf-8') as f:
        # 定义一个辅助函数，同时在控制台和文件中输出
        def write_line(text):
            print(text)
            f.write(text + '\n')

        # --- 第1部分：输出各节点基准水头计算详情 (此前省略部分) ---
        write_line("正在输出各节点基准水头详细信息...")
        import sys
        from io import StringIO
        original_stdout = sys.stdout
        captured_output = StringIO()
        sys.stdout = captured_output  # 临时重定向标准输出
        irrigation_system.print_node_baseline_heads()  # 调用内置的打印函数
        sys.stdout = original_stdout  # 恢复标准输出
        baseline_output = captured_output.getvalue()
        print(baseline_output, end='')  # 在控制台显示
        f.write(baseline_output)  # 写入文件

        # --- 第2部分：输出斗管分段管径配置 (此前省略部分) ---
        write_line("\n=== 斗管分段管径配置 ===")
        ...
        cost_per_area = total_cost / change_area if change_area > 0 else 0
        ...
        write_line(f"干管总长度: {total_long:.1f} m")
...
def visualize_pareto_front(pareto_front):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('NSGA_DAN_YOUHUA_pareto_front.png', dpi=300, bbox_inches='tight')
    except Exception as e:
    ...
    try:
        ...
        if auto_save == 1:
            ...
            for i, fig in enumerate(figures):
                try:
                    fig.savefig(f'NSGA_DAN_YOUHUA_result_fig{i}.png', dpi=300, bbox_inches='tight')
                    logging.info(f"图表{i}已保存为 NSGA_DAN_YOUHUA_result_fig{i}.png")
                except Exception as e:
