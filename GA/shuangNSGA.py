...
def multi_objective_optimization(irrigation_system, lgz1, lgz2, swarm_size, max_iterations, show_plots, auto_save):
    ...
    try:
        ...
        save_pareto_front(pareto_front_values, "NSGA_SHUANG_Verified")
        save_pareto_solutions(verified_pareto_front, "NSGA_SHUANG_Verified")
        print("验证通过的帕累托解集已成功保存")

        # 如果有失败的个体，也保存一份用于分析
        if failed_individuals:
            failed_values = np.array([ind.fitness.values for ind in failed_individuals])
            save_pareto_front(failed_values, "NSGA_SHUANG_Failed")
            save_pareto_solutions(failed_individuals, "NSGA_SHUANG_Failed")
...
def save_pareto_front(pareto_front, algorithm_name, save_dir='NSGA_SHUANG_result'):
    """
...
def save_pareto_solutions(solutions, algorithm_name, save_dir='NSGA_SHUANG_result'):
    """
...
def print_detailed_results(irrigation_system, best_individual, output_file="optimization_results_NSGAⅡ_SHUANG.txt"):
