...
class PSOOptimizationTracker:
    ...
    def finalize_plots(self):
        ...
        try:
            ...
            # 如果需要，保存图表
            if self.auto_save:
                self.fig_2d.savefig('PSO_DAN_YOUHUA_2d_curves.png', dpi=300, bbox_inches='tight')
                self.fig_3d.savefig('PSO_DAN_YOUHUA_3d_progress.png', dpi=300, bbox_inches='tight')
    ...
    def plot_2d_curves(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('PSO_DAN_YOUHUA_2d_curves.png', dpi=300, bbox_inches='tight')
    ...
    def plot_3d_progress(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('PSO_DAN_YOUHUA_3d_progress.png', dpi=300, bbox_inches='tight')
...
def multi_objective_pso(irrigation_system, lgz1, lgz2, swarm_size, max_iterations, show_plots,
    ...
    def evaluate(position):
        """
        评估函数 - PSO版本，适用于【原始方案：头尾对开】，并采用【全局合并方差法】
        说明：
        此函数是PSO算法的核心，用于计算每个粒子的帕累托目标值。
        ...
        try:
            ...
            if final_corrected_position is not None:
                ...
                _update_pipe_diameters(irrigation_system, position.tolist())
                final_corrected_position = encode_pipe_diameters_from_system_pso(irrigation_system)
                position[:] = final_corrected_position

            # 步骤二：使用最终确定的管径方案，计算成本和全局压力均方差
            _update_pipe_diameters(irrigation_system, final_corrected_position.tolist())
    ...
    try:
        ...
        save_pareto_front(pareto_front_values, "PSO_DAN_YOUHUA_Verified")
        save_pareto_solutions(verified_pareto_front, "PSO_DAN_YOUHUA_Verified")
        print("验证通过的帕累托解集已成功保存")

        # 如果有失败的粒子，也保存一份用于分析
        if failed_particles:
            failed_values = np.array([p.best_fitness for p in failed_particles])
            save_pareto_front(failed_values, "PSO_DAN_YOUHUA_Failed")
            save_pareto_solutions(failed_particles, "PSO_DAN_YOUHUA_Failed")
...
def save_pareto_front(pareto_front, algorithm_name, save_dir='PSO_DAN_YOUHUA_result'):
    """
...
def save_pareto_solutions(solutions, algorithm_name, save_dir='PSO_DAN_YOUHUA_result'):
    """
...
def print_detailed_results(irrigation_system, best_solution,
                           output_file="optimization_results_PSO_DAN_YOUHUA.txt"):
    """
    PSO版本：优化后的结果输出函数 - 【完整通用版】
    说明：
    - 本版本包含了所有输出细节，无任何省略。
...
def visualize_pareto_front(pareto_front_particles):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('PSO_DAN_YOUHUA_pareto_front.png', dpi=300, bbox_inches='tight')
    except Exception as e:
    ...
    try:
        ...
        if auto_save == 1:
            ...
            for i, fig in enumerate(figures):
                try:
                    fig.savefig(f'PSO_DAN_YOUHUA_result_fig{i}.png', dpi=300, bbox_inches='tight')
                    logging.info(f"图表{i}已保存为 PSO_DAN_YOUHUA_result_fig{i}.png")
                except Exception as e:
