...
def extract_and_save_pareto_data(pareto_front, irrigation_system, pipe_specs):
    ...
    for particle in pareto_front:
        if np.all(np.isfinite(particle.best_fitness)):
            # 解码管径配置
            decoded_config = decode_pipe_configuration(list(particle.position), irrigation_system, pipe_specs)
            solution = {
                'cost': particle.best_fitness[0],
                'variance': particle.best_fitness[1],
                'raw_configuration': list(particle.position),
