...
class PSOOptimizationTracker:
    ...
    def _init_plots(self):
        ...
        self.ax1.set_title('丰字PSO算法优化迭代曲线', fontproperties=chinese_font, fontsize=chinese_size + 2)
    ...
    # 内部辅助方法 - 确保与全局函数完全一致
    def _select_best_solution(self, solutions, max_variance_threshold=4.0):
        """
    ...
    def _update_plots(self):
        ...
        try:
            ...
            # 更新标题，反映这是最优个体的迭代曲线
            self.ax1.set_title('丰字PSO算法优化最优个体迭代曲线', fontproperties=chinese_font,
    ...
    def plot_2d_curves_per_area(self, irrigation_system):
        """绘制基于每亩成本的2D迭代曲线"""
        if not self.iterations:
            print("没有数据可供绘图")
            return
        ...
        # 转换为每亩成本
        cost_per_area_data = [cost / change_area for cost in cost_data]

        if self.enable_smoothing and len(self.iterations) > 5:
            # 应用平滑算法 - 最终图表使用更大的窗口
            smoothed_cost = self._smooth_curve(cost_per_area_data, window_size=25)
            ...
            smoothed_cost = cost_per_area_data
            smoothed_variance = variance_data

        # 成本曲线 - 移除标记点，只使用线条
        ax1.plot(self.iterations, smoothed_cost, 'g-', linewidth=2)
        ax1.set_ylabel('单位面积成本 (元/亩)', fontproperties=chinese_font, fontsize=chinese_size)
        ax1.set_title('丰字PSO算法优化迭代曲线 (单位面积成本)', fontproperties=chinese_font,
                      fontsize=chinese_size + 2)
        ...
        # 保存图表
        if self.auto_save:
            plt.savefig('PSO_SHUANG_2d_curves_per_area.png', dpi=300, bbox_inches='tight')

        # 显示图表
        plt.ion()  # 重新开启交互模式以便能打开多个图表
        plt.show(block=False)

    def plot_3d_progress_per_area(self, irrigation_system):
        """绘制基于每亩成本的3D进度图"""
        ...
        # 转换为每亩成本
        costs_per_area = [cost / change_area for cost in self.all_costs]
        ...
        # 设置轴标签
        ax.set_xlabel('单位面积成本 (元/亩)', fontproperties=chinese_font, fontsize=chinese_size)
        ...
        # 设置图表标题
        ax.set_title('丰字PSO算法优化3D进度图 (单位面积成本)', fontproperties=chinese_font, fontsize=chinese_size + 2)
        ...
        # 保存图表
        if self.auto_save:
            plt.savefig('PSO_SHUANG_3d_progress_per_area.png', dpi=300, bbox_inches='tight')
    ...
    def plot_2d_curves(self):
        ...
        ax1.set_title('丰字PSO算法优化迭代曲线', fontproperties=chinese_font, fontsize=chinese_size + 2)
        ax1.grid(True, linestyle='--', alpha=0.7)
    ...
    def plot_3d_progress(self):
        ...
        # 设置图表标题
        ax.set_title('丰字PSO算法优化3D进度图', fontproperties=chinese_font, fontsize=chinese_size + 2)
...
def visualize_pareto_front(pareto_front_particles):
    ...
    try:
        ...
        plt.title('多目标丰字PSO管网优化Pareto前沿', fontproperties=chinese_font, fontsize=chinese_size + 2, pad=15)
        ...
        plt.tight_layout()
        plt.savefig('PSO_SHUANG_pareto_front.png', dpi=300, bbox_inches='tight')
    except Exception as e:
        logging.error(f"可视化过程中发生错误: {str(e)}")
        print(f"可视化失败: {str(e)}")


def visualize_pareto_front_per_area(pareto_front_particles, irrigation_system):
    """可视化基于每亩成本的Pareto前沿"""
    try:
        if not pareto_front_particles:
            print("没有可视化的解集")
        ...
        for particle in pareto_front_particles:
            if np.all(np.isfinite(particle.best_fitness)):
                # 转换为每亩成本
                total_cost = particle.best_fitness[0]
                cost_per_area = total_cost / change_area
                costs_per_area.append(cost_per_area)
                variances.append(particle.best_fitness[1])

        if not costs_per_area or not variances:
            print("没有有效的解集数据可供可视化")
            return

        plt.figure(figsize=(10, 6), dpi=100)
        plt.scatter(costs_per_area, variances, c='green', marker='o', s=50, alpha=0.6, label='Pareto解')

        plt.title('多目标丰字PSO管网优化Pareto前沿 (单位面积成本)', fontproperties=chinese_font,
                  fontsize=chinese_size + 2, pad=15)
        plt.xlabel('单位面积成本 (元/亩)', fontproperties=chinese_font, fontsize=chinese_size)
        plt.ylabel('压力方差', fontproperties=chinese_font, fontsize=chinese_size)
        plt.grid(True, linestyle='--', alpha=0.7)
        ...
        # 保存图像
        plt.savefig('PSO_SHUANG_pareto_front_per_area.png', dpi=300, bbox_inches='tight')

    except Exception as e:
        logging.error(f"单位面积成本可视化过程中发生错误: {str(e)}")
        print(f"单位面积成本可视化失败: {str(e)}")


def select_best_solution_by_marginal_improvement(solutions, max_variance_threshold=4.0):
    """
    ...
    try:
        ...
        if pareto_front:
            ...
            if valid_solutions:
                ...
                # 新增：生成并显示每亩成本图像
                visualize_pareto_front_per_area(pareto_front, irrigation_system)

                # 新增：每亩成本的2D和3D进度图
                tracker.plot_2d_curves_per_area(irrigation_system)
                tracker.plot_3d_progress_per_area(irrigation_system)
        ...
        if auto_save == 1:
            ...
            for i, fig in enumerate(figures):
                try:
                    fig.savefig(f'PSO_SHUANG_result_fig{i}.png', dpi=300, bbox_inches='tight')
                    logging.info(f"图表{i}已保存为 PSO_SHUANG_result_fig{i}.png")
                except Exception as e:
