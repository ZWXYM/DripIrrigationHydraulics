...
def friction_factor(diameter, flow_rate):
    d = diameter / 1000
    Re = 1000 * flow_rate / ((d / 2) ** 2 * math.pi) * d / 1.004e-3  # 雷诺数1000是密度，1.004e-3是粘滞系数pas
...
def pressure_loss(diameter, length, flow_rate):
    ...
    v = flow_rate / (math.pi * (d / 2) ** 2)  # 流速(m/s)
    h_f = f * (length / d) * (v ** 2 / (2 * 9.81))  # 达西-魏斯巴赫公式计算压力损失(m)
    return h_f


# 计算流量
def calculate_flow_rates(dripper_min_flow, dripper_length, dripper_spacing, fuzhu_sub_length, lgz1):
    ...
    lateral_flow = dripper_flow * math.floor(fuzhu_sub_length / dripper_distance) * 2 * 1 * num_drippers
...
def calculate_head(sub_diameter, lateral_diameter, length_x, length_y, dripper_min_flow, dripper_length,
    ...
    print(f"农管轮灌最大流速: {lateral_flow / ((lateral_diameter / 1000 / 2) ** 2 * math.pi):.3f} m/s")
    print(f"斗管雷诺数: {Re_sub:.3f}")
    print(f"斗管沿程阻力系数: {f_sub:.3f}")
    print(f"斗管水头损失: {sub_loss:.3f} m")
    print(f"斗管轮灌最大流量: {sub_flow:.3f} m³/s")
    print(f"斗管轮灌最大流速: {sub_flow / ((sub_diameter / 1000 / 2) ** 2 * math.pi):.3f} m/s")
    print(f"支管轮灌最大流速: {sub_flow * lgz2 / ((600 / 1000 / 2) ** 2 * math.pi):.3f} m/s")  # 开启多少条斗管
...
def shuili(dripper_length, dripper_spacing, dripper_min_flow, fuzhu_sub_length, field_length, Soil_bulk_density,
           field_z,
           field_p, field_max, field_min, sr, st, ib, nn):
...
if __name__ == "__main__":
    calculate_head(160, 90, 180, 750, 2.1, 50, 0.3, 40, 4, 12)
    shuili(50, 0.3, 2.1, 40, 100,
           1.46, 600,
           0.7, 0.9, 0.8, 0.8, 0.1, 8, 0.9)  # m;m;L\h;m;M;G\CM3;MM;%;%;%;M;M;MM\D;%
=======
import math
from pylab import mpl
