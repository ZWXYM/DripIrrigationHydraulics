...
class NSGAOptimizationTracker:
    ...
    def plot_2d_curves_per_area(self, irrigation_system):
        ...
        # 成本曲线 - 移除标记点，只使用线条
        ax1.plot(self.generations, smoothed_cost, 'g-', linewidth=2)
        ax1.set_ylabel('单位面积成本 (元/亩)', fontproperties=chinese_font, fontsize=chinese_size)
        ax1.set_title('梳齿状NSGA-II算法优化迭代曲线 (单位面积成本)', fontproperties=chinese_font,
        ...
        # 保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_2d_curves_per_area.png', dpi=300, bbox_inches='tight')
    ...
    def plot_3d_progress_per_area(self, irrigation_system):
        ...
        # 设置图表标题
        ax.set_title('梳齿状NSGA-II算法优化3D进度图 (单位面积成本)', fontproperties=chinese_font,
                     fontsize=chinese_size + 2)
        ...
        # 保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_3d_progress_per_area.png', dpi=300, bbox_inches='tight')
    ...
    def finalize_plots(self):
        ...
        try:
            ...
            # 如果需要，保存图表
            if self.auto_save:
                self.fig_2d.savefig('NSGA_DAN_2d_curves.png', dpi=300, bbox_inches='tight')
                self.fig_3d.savefig('NSGA_DAN_3d_progress.png', dpi=300, bbox_inches='tight')
    ...
    def plot_2d_curves(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_2d_curves_smoothed.png', dpi=300, bbox_inches='tight')
    ...
    def plot_3d_progress(self):
        ...
        # 如果需要，保存图表
        if self.auto_save:
            plt.savefig('NSGA_DAN_3d_progress.png', dpi=300, bbox_inches='tight')
...
def visualize_pareto_front(pareto_front):
    ...
    try:
        ...
        plt.tight_layout()
        plt.savefig('NSGA_DAN_pareto_front.png', dpi=300, bbox_inches='tight')
    except Exception as e:
        logging.error(f"可视化过程中发生错误: {str(e)}")
        print(f"可视化失败: {str(e)}")


def visualize_pareto_front_per_area(pareto_front, irrigation_system):
    """可视化基于每亩成本的Pareto前沿"""
    try:
        if not pareto_front:
            print("没有可视化的解集")
        ...
        for ind in pareto_front:
            if np.all(np.isfinite(ind.fitness.values)):
                # 转换为每亩成本
                total_cost = ind.fitness.values[0]
                cost_per_area = total_cost / change_area
        ...
        plt.title('多目标丰字NSGAⅡ管网优化Pareto前沿 (单位面积成本)', fontproperties=chinese_font,
        ...
        # 保存图像
        plt.savefig('NSGA_DAN_pareto_front_per_area.png', dpi=300, bbox_inches='tight')
    ...
    try:
        ...
        if auto_save == 1:
            ...
            for i, fig in enumerate(figures):
                try:
                    fig.savefig(f'NSGA_DAN_result_fig{i}.png', dpi=300, bbox_inches='tight')
                    logging.info(f"图表{i}已保存为 NSGA_DAN_result_fig{i}.png")
                except Exception as e:
